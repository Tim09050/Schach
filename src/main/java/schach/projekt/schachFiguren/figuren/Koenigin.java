    ...
    public ArrayList<Feld> getGueltigenZug() {
        ...
        for (Integer vektor : vektoren) {
            while (true) {
                ...
                if (Schachbrett.felder[jetzigesFeldIndex].getFigurAufFeld() != null && Schachbrett.felder[jetzigesFeldIndex].getFigurAufFeld().getTeam() == this.getTeam())
                    break;
                if (Schachbrett.felder[jetzigesFeldIndex].getFigurAufFeld() != null && Schachbrett.felder[jetzigesFeldIndex].getFigurAufFeld().getTeam() != this.getTeam()) {
                ...
                //diagonale Bewegung
                if(((jetzigesFeldIndex) % 8 == 0 || (jetzigesFeldIndex + 1) % 8 == 0) && (Math.abs(vektor) == 7 || Math.abs(vektor) == 9)){
                    gueltigerZug.add(Schachbrett.felder[jetzigesFeldIndex]);
                    break;
                }
                //Restricting horizontal movement
                if((((jetzigesFeldIndex) % 8 == 0) || (jetzigesFeldIndex + 1) % 8 == 0) && Math.abs(vektor) == 1){
                    gueltigerZug.add(Schachbrett.felder[jetzigesFeldIndex]);
                    break;
                }
